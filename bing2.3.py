...
# -*- coding: utf-8 -*-
"""
Bing Rewards 自动化脚本 - 多账号分离版 v2.3.3

包含以下功能:
1. 多账号并发执行 (基于 ThreadPoolExecutor 线程池，支持并发数控制)
2. 全任务自动覆盖 (搜索、更多活动、阅读任务)
3. 智能防风控系统 (拟人化搜索间隔、真实热搜词库、随机 UA)
4. 独享代理支持 (全局锁控制提取频率、支持 HTTP/SOCKS5、支持单账号独立开关)

更新说明:

### 20260125
v2.3.3:
- 🔄 规则适配：移除移动端搜索功能（微软已取消双端搜索机制，改为统一搜索积分）。
- 🧹 界面优化：移除推送通知中的"移动"列，改为单独"搜索"列。

### 20260118
v2.3.2:
- 🚀 并发重构：引入线程池 (ThreadPoolExecutor) 替代原始多线程，资源调度更优。
- 🛡️ 代理升级：新增全局锁机制 (Global Lock)，解决多线程并发提取导致代理商API限频报错的问题。
- 🔌 协议支持：新增 SOCKS5/HTTP 协议切换支持 (变量 BING_PROXY_TYPE)。
- ⚙️ 精细控制：新增 bing_proxy_N 环境变量，可单独控制特定账号是否走代理（支持混合模式）。
- 📊 体验优化：推送通知改为结构化表格展示，支持 "备注#Cookie" 格式显示中文别名。

### 20260112
v2.3.1:
- 🐛 修复 400 错误：彻底修复网页端"更多活动"任务上报失败的问题。
- 🔧 底层重构：重写 RequestManager，实现 Session 持久化与 User-Agent 固定。

配置说明:
1. 基础配置 (环境变量):
   - bing_ck_1, bing_ck_2... : 必应 Cookie (必需)
     * 支持备注格式: "我的大号#cookie内容..."
   - bing_token_1, bing_token_2... : 刷新令牌 (可选，用于 APP 阅读任务)

2. 代理配置 (环境变量 或 脚本内修改):
   - BING_PROXY_URL  : 代理提取链接 (支持品赞等，返回 JSON 或 Text)
   - BING_PROXY_TYPE : 代理协议，可选 "http" (默认) 或 "socks5" (需安装 PySocks)
   - bing_proxy_n    : (可选) 单独控制账号n是否走代理。
                       设置 "false" 则强制该账号直连；不设置默认跟随全局。

3. 脚本内配置 (TASK_CONFIG):
   - CONCURRENCY     : 并发线程数 (默认为3)
   - MAX_REPEAT_COUNT: 每日最大重复运行次数 (防止过度运行)

Cookie 获取提示:
1. 登录 https://cn.bing.com/
2. 访问 https://rewards.bing.com/welcome 确认账号一致
3. F12 抓取完整 Cookie，确保包含 tifacfaatcs 和 .MSA.Auth 字段

⚠️ 依赖安装:
pip3 install requests
pip3 install PySocks (如果使用 socks5 代理)
...
# ================= 随机延时模块 Start =================
# 设置延时范围（单位：秒）
min_delay = 20   # 最少等待 10 秒
max_delay = 60  # 最多等待 300 秒 (5分钟)

# 生成随机数
delay_seconds = random.randint(min_delay, max_delay)

print(f"【防封机制】脚本将随机延时 {delay_seconds} 秒后启动...")
...
TASK_CONFIG = {
    ...
    'SEARCH_DELAY_MIN': 45,          # 搜索延迟最小值（秒）
    'SEARCH_DELAY_MAX': 95,          # 搜索延迟最大值（秒）
	...
	'CONCURRENCY': 3,                # 并发线程数 (默认为3，建议不要超过5，防止IP黑号)
}
...
# ==================== 代理配置 (新增) ====================
# 品赞代理提取链接 (留空则使用本地网络)
# 格式要求: 提取数量1, 格式文本(txt)或JSON, 分隔符换行(\n), 地区建议选全国
# [修改] 优先读取环境变量，如果没配则为空
BING_PROXY_URL = os.getenv("BING_PROXY_URL", "")

# [新增] 代理协议类型
# 可选值: "http" 或 "socks5"
# 注意: 如果使用 socks5，必须先运行 pip3 install PySocks
# [修改] 优先读取环境变量，默认 http
BING_PROXY_TYPE = os.getenv("BING_PROXY_TYPE", "http")
# [新增] 全局代理锁，用于控制提取频率
PROXY_EXTRACT_LOCK = threading.Lock()
...
class Config:
    """配置类，统一管理所有配置项"""
	# 并发配置
    CONCURRENCY: int = TASK_CONFIG['CONCURRENCY']
    # 搜索配置
...
class AccountManager:
    ...
    def get_accounts() -> List[AccountInfo]:
        ...
        while index <= max_check_index:
            raw_cookie = os.getenv(f"bing_ck_{index}")
            refresh_token = os.getenv(f"bing_token_{index}", "")
            
			# [新增] 读取代理开关配置
            # 默认为 "true"，除非显式设置为 false/0/no/off
            proxy_config = os.getenv(f"bing_proxy_{index}", "true").lower()
            use_proxy_flag = proxy_config not in ["false", "0", "no", "off"]
            # [新增] 解析备注名
            alias = f"账号{index}"
            cookies = raw_cookie
            ...
            if raw_cookie and '#' in raw_cookie:
                try:
                    # 分割 备注#Cookie
                    alias_part, cookie_part = raw_cookie.split('#', 1)
                    alias = alias_part.strip()
                    cookies = cookie_part.strip()
                except:
                    pass

            # 下面是原有的验证逻辑，注意要把原来的 cookies 变量引用改成新的 cookies
            ...
            if not cookies:
                # ... (保留原有逻辑，无需变动)
                print_log("账号配置", f"{alias} 缺少cookies配置，跳过", index)
                # ...
                index += 1
                continue
            ...
            accounts.append(AccountInfo(
				...
				use_proxy=use_proxy_flag  # [新增] 传入开关状态
            ))
...
class ProxyManager:
    """品赞代理管理器 (支持 全局锁 / 协议切换 / 双格式解析)"""
    
    @staticmethod
    def format_proxy_log(proxy_str: str) -> str:
        """脱敏输出日志"""
        try:
            if '@' in proxy_str:
                return f"***:***@{proxy_str.split('@')[1]}"
            return proxy_str
        except:
            return proxy_str

    @staticmethod
    def fetch_proxy(account_index: int) -> Optional[str]:
        """提取代理IP (线程安全 + 协议自动识别)"""
        # 1. 检查配置
        if not BING_PROXY_URL:
            return None

        # 2. 全局锁控制：只有拿到锁的线程才能提取，其他线程排队
        with PROXY_EXTRACT_LOCK:
            print_log("代理状态", f"账号[{account_index}] 正在排队提取代理...", account_index)
            
            try:
                # 请求 API
                resp = requests.get(BING_PROXY_URL, timeout=10)
                
                if resp.status_code != 200:
                    print_log("提取失败", f"HTTP状态码: {resp.status_code}", account_index)
                    return None

                content = ""
                # --- 智能解析逻辑 (支持 JSON 和 纯文本) ---
                try:
                    # 尝试 JSON 解析
                    json_data = resp.json()
                    if isinstance(json_data, dict):
                        if 'code' in json_data and json_data['code'] not in [0, 200] and json_data.get('success') is not True:
                             print_log("提取失败", f"API报错: {json_data.get('msg') or json_data}", account_index)
                             return None
                        ...
                        if data_list and isinstance(data_list, list) and len(data_list) > 0:
                            item = data_list[0]
                            content = f"{item.get('ip')}:{item.get('port')}"
                        elif 'ip' in json_data and 'port' in json_data:
                            content = f"{json_data['ip']}:{json_data['port']}"
                            
                except ValueError:
                    # JSON 解析失败，回退到文本模式 (例如: 1.1.1.1:8888)
                ...
                if content and ':' in content:
                    # 读取协议配置 (http 或 socks5)
                    p_type = os.getenv("BING_PROXY_TYPE", BING_PROXY_TYPE).lower()
                    
                    if "socks" in p_type:
                        proxy = f"socks5://{content}"
                    else:
                        proxy = f"http://{content}"

                    print_log("代理获取", f"🔍 提取成功 [{p_type}]: {ProxyManager.format_proxy_log(content)}", account_index)
                    
                    # [关键] 强制冷却 1.5秒，防止频繁提取导致 API 报错
                    time.sleep(1.5)
                    
                    return proxy
                else:
                    print_log("提取失败", f"未能解析代理: {resp.text[:50]}", account_index)
                    return None

            except Exception as e:
                print_log("提取异常", str(e), account_index)
                return None

    @staticmethod
    def test_proxy(proxy_url: str, account_index: int) -> bool:
        """握手测试 (在锁外执行，允许并发测试)"""
        try:
            start = time.time()
            # 访问百度测试连通性
            requests.get("https://www.baidu.com", proxies={"http": proxy_url, "https": proxy_url}, timeout=5)
            cost = int((time.time() - start) * 1000)
            return True
        except:
            print_log("代理测试", "❌ 握手失败，代理无效", account_index)
            return False

# ==================== HTTP请求管理 ====================
class RequestManager:
    """HTTP请求管理器 - 智能代理版 (集成固定UA)"""
    
    # [修改] 增加 enable_proxy 参数，默认为 True
    def __init__(self, account_index: Optional[int] = None, enable_proxy: bool = True):
        self.session = requests.Session()
        self.account_index = account_index
        # 保持固定 UA
        self.pc_ua = config.get_random_pc_ua()
        self.mobile_ua = config.get_random_mobile_ua()
        
        # 代理逻辑: 
        # 1. 全局必须配置了 URL
        # 2. 账号单独的开关必须为 True (enable_proxy)
        self.use_proxy = bool(BING_PROXY_URL and BING_PROXY_URL.strip()) and enable_proxy
        
        self.current_proxy = None
        
        if self.use_proxy:
            if account_index is not None:
                # 初始提取失败则报错终止，防止泄漏本地IP
                if not self._refresh_proxy():
                    raise Exception("代理初始化失败，已终止运行（防本地IP泄漏）")
        else:
            if BING_PROXY_URL:
                # 如果全局有代理但此账号禁用了，打印个提示
                # 注意：这里不能用 print_log，因为 logger 可能还没初始化完全，或者为了简洁直接 print
                pass

    def _refresh_proxy(self) -> bool:
        """刷新/获取代理"""
        for _ in range(3): 
            proxy = ProxyManager.fetch_proxy(self.account_index)
            if proxy and ProxyManager.test_proxy(proxy, self.account_index):
                self.current_proxy = {"http": proxy, "https": proxy}
                self.session.proxies.update(self.current_proxy)
                return True
            time.sleep(2)
        return False

    def get_browser_headers(self, cookies: str) -> Dict[str, str]:
        return {
            "user-agent": self.pc_ua, # 使用固定 PC UA
    ...
    def get_mobile_headers(self, cookies: str) -> Dict[str, str]:
        return {
            "user-agent": self.mobile_ua, # 使用固定 Mobile UA
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
    ...
    def make_request(self, method: str, url: str, headers: Dict[str, str], 
        ...
        if not self.use_proxy:
            ...
            return self._post_request(url, headers, data, timeout)
        ...
        for i in range(2): 
            try:
                ...
                if i == 0: 
                    print_log("网络波动", f"⚠️ 代理不稳定，尝试热切换...", account_index)
                    if self._refresh_proxy():
                        print_log("热切换", "✅ 新代理生效，重试...", account_index)
                        continue
                raise e 
        
        raise ValueError(f"Unknown method: {method}")

    def _post_request(self, url, headers, data, timeout):
        """内部POST处理"""
...
# ==================== 主要业务逻辑类 ====================
class RewardsService:  # <--- 补上这一行！！！！！！
    """Microsoft Rewards服务类"""

    # ==================== 1. 基础设施方法 ====================
    # [修改] 方法签名增加 enable_proxy 参数，默认为 True
    def __init__(self, account_index: Optional[int] = None, enable_proxy: bool = True): 
        """初始化服务"""
        
        # [修改] 关键点：将 enable_proxy 参数透传给 RequestManager
        # 这样 RequestManager 才能知道这个账号是否被允许使用代理
        self.request_manager = RequestManager(account_index, enable_proxy) 
        
        # 初始化各个管理器 (原来的代码里这里重复了，我帮你精简保留了一份)
    ...
    def _report_activity(self, task: Dict[str, Any], token: str, cookies: str, account_index: Optional[int] = None) -> bool:
        ...
        try:
            # 1. 基础数据准备
            offer_id = task.get('offerId')
            if not offer_id:
                offer_id = task.get('name')
            ...
            # 2. 关键修复：手动将静态Cookie注入到Session中，而不是通过Header覆盖
            # 这样可以保留Session中服务器下发的新Cookie
            ...
            # 将静态Cookie合并到当前的Session CookieJar中
            self.request_manager.session.cookies.update(cookie_dict)

            # 3. 构建Header（注意：这里不再包含 'cookie' 字段，交由Session自动管理）
            # 获取基础Header但移除Cookie字段
            base_headers = self.request_manager.get_browser_headers(cookies)
            if 'cookie' in base_headers:
                del base_headers['cookie']
            if 'Cookie' in base_headers:
                del base_headers['Cookie']
            ...
            base_headers.update({
                'Accept': 'application/json, text/javascript, */*; q=0.01',
                'Content-Type': 'application/x-www-form-urlencoded', # 移除 charset 防止兼容性问题
                'X-Requested-With': 'XMLHttpRequest',
                'Origin': 'https://rewards.bing.com',
                'Referer': 'https://rewards.bing.com/dashboard', # 修改 Referer 为 dashboard
                'Sec-Fetch-Dest': 'empty',
                'Sec-Fetch-Mode': 'cors',
                'Sec-Fetch-Site': 'same-origin',
            })
            ...
            # 注意：这里 make_request 会使用 self.session，此时 session.cookies 已经包含了所有必要的 cookie
            response = self.request_manager.session.post(
                post_url, 
            ...
            if response.status_code == 200:
                try:
                    result = response.json()
                    if result.get("activity") and "points" in result["activity"]:
                        points = result['activity']['points']
                        print_log("任务奖励", f"✅ 获得{points}积分", account_index)
                        return True
                    else:
                        # 很多时候成功了返回的内容不包含 points，但也算成功
                        print_log("任务奖励", "✅ 任务完成 (无即时积分返回)", account_index)
                        return True
                except:
                    # 只要200 OK，大概率是成功的
                    print_log("任务奖励", "✅ 任务可能已完成 (解析响应失败)", account_index)
                    return True
            else:
                # 打印前100个字符的错误信息
                error_preview = response.text[:100].replace('\n', ' ')
                print_log("活动报告", f"❌ 失败 | 状态码: {response.status_code} | 响应: {error_preview}", account_index)
                return False
...
class RewardsBot:
    ...
    def _perform_search_tasks(self, cookies: str, account_index: int, email: str, service: RewardsService, stop_event: threading.Event):
        ...
        # 移动搜索 - 微软已取消双端搜索机制，统一为单一搜索积分
        # 2026.01.25: 根据微软奖励官网最新规则，不再区分PC端和移动端搜索
        logger.skip("移动搜索", "微软已取消双端搜索，无需执行", account_index)
    ...
    def _format_account_summary(self, email: str, start_points: int, final_points: int, 
                               ...
                               today_total_earned: int = 0, app_sign_in_points: int = 0, access_token: Optional[str] = None) -> Dict[str, Any]:
        """
        [修改版] 返回结构化数据字典 (强制显示具体分数，去掉勾选标记)
        """
        # 1. 邮箱脱敏
        masked_email = email
        if '@' in email:
        ...
        # 辅助函数：格式化进度 (修改点：不再返回 ✅，而是始终返回 数字/数字)
        def fmt_prog(curr, total):
            if total == 0: return "➖"
            return f"{curr}/{total}"
        ...
        pc_status = "❓"
        read_status = "❓"
        more_status = f"{more_completed}" # 更多活动一般没有总数，暂保持原样或显示数量
        ...
        try:
            ...
            if dashboard_data and dashboard_data.get('dashboard'):
                ...
                pc_search_tasks = counters.get("pcSearch", [])
                if pc_search_tasks:
                    prog = sum(t.get('pointProgress', 0) for t in pc_search_tasks)
                    maxn = sum(t.get('pointProgressMax', 0) for t in pc_search_tasks)
                    pc_status = fmt_prog(prog, maxn)
                else:
                    pc_status = "无数据"

                # --- 更多活动 (尝试获取总数) ---
                # 这里简单处理，如果想显示 x/y，需要前面的逻辑支持，目前保持完成数
                # 或者如果有 more_total 参数传入，可以修改这里。目前暂时显示完成数量。
                if 'more_total' in locals() and more_total > 0:
                     more_status = f"{more_completed}/{more_total}"
                else:
                     more_status = f"{more_completed}"
        ...
        # --- 阅读任务状态 (修改点：始终显示进度) ---
        if account.refresh_token:
            read_status = f"{read_completed}/30"
        else:
            read_status = "🚫Token"
    ...
    def run(self):
        ...
        print_log("系统启动", f"启动并发线程池，最大并发数: {max_workers}")
        ...
        # 定义单个任务的包装函数
        def _worker(account: AccountInfo):
            print(f"🔄 正在初始化账号[{account.index}]...")
            service = None
            stop_event = threading.Event()
            
            try:
                # [修改] 初始化服务时，传入账号的代理开关状态
                service = RewardsService(account.index, enable_proxy=account.use_proxy)
                
                # 执行任务
                summary = self.process_single_account(account, service, stop_event)
                
                # 检查是否因搜索失败触发了停止事件
                is_failed = stop_event.is_set()
                ...
                # 搜索任务失败导致的线程终止（通常在 process_single_account 内部处理了，但为了保险）
                return {
                    'index': account.index,
                    'summary': None,
                    'failed': True
                ...
                print_log(f"账号{account.index}错误", f"线程执行异常: {e}", account.index)
                return {
                    'index': account.index,
                    'summary': None,
                    'failed': True # 异常也视为失败
                ...
                # 确保资源释放
                if service and hasattr(service, 'request_manager'):
                    service.request_manager.close()

        # 使用线程池执行任务
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有账号任务
            future_to_account = {executor.submit(_worker, account): account for account in self.accounts}
            ...
            for future in as_completed(future_to_account):
                ...
                try:
                    ...
                    print_log(f"账号{account.index}异常", f"任务生成的异常: {exc}", account.index)
        ...
        for res in results:
            if res['summary']:
                sorted_summaries.append(res['summary'])
        
        # 推送结果
        self._send_notification(sorted_summaries, any_search_failed)
    
    def _send_notification(self, summaries: List[Dict[str, Any]], any_search_failed: bool):
        """发送通知 (表格化版 - 修复对齐/显示分数/合并账号名)"""
        ...
        # 这里的空格是为了让短标题在长列中看起来居中一点，纯视觉优化
        header = (
            pad_text("   账号", W_NAME) +    # 前面加3个空格
            pad_text(" 总分(今日)", W_POINTS) + # 前面加1个空格
            pad_text("搜索", W_PC) + 
            pad_text("阅读", W_READ) + 
            pad_text("更多", W_MORE)
        )
        ...
        for s in summaries:
            # 组合名称: 备注(邮箱)
            full_name = f"{s['name']}({s['email']})"
            
            # 截断处理 (防止爆表格)
            if get_display_width(full_name) > W_NAME - 2:
                 full_name = full_name[:20] + ".."

            points_display = f"{s['points']}({s['today']})"
            
            # 构建控制台输出行
            line_str = (
                pad_text(full_name, W_NAME) + 
                pad_text(points_display, W_POINTS) + 
                pad_text(s['pc'], W_PC) + 
                pad_text(s['read'], W_READ) + 
                pad_text(s['more'], W_MORE)
            )
            print(line_str)
            ...
            # 格式: 👤备注(邮箱) | 💰1596(+174)
            #       🔍90/90 📖30/30 🎯2
            msg_line = f"👤{full_name} | 💰{points_display}\n   🔍{s['pc']}  📖{s['read']}  🎯{s['more']}"
        ...
        if any_search_failed:
            print_log(f"统一推送", "任务未全部完成 (存在搜索失败)，跳过推送")
            return

        print_log(f"统一推送", "任务全部完成，准备推送...")
